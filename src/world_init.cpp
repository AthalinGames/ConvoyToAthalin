...
void realignCards() {
	...
	if (!current_cards.empty()) {
		const float card_offset = CARD_AXIS_WIDTH / (static_cast<float>(current_cards.size()) + 1);
		const float card_z_offset = Z_FOREGROUND / (current_cards.size() + 3);
		auto &first_card = registry.stationaries.get(registry.cards.entities[0]);
		auto &first_card_render = registry.renderForeground.get(registry.cards.entities[0]);
		first_card.position = vec2(card_offset,
		                           CARD_AXIS_HEIGHT);
		first_card.scale = vec2(CARD_WIDTH, CARD_HEIGHT);
		first_card_render.z_position = Z_FOREGROUND - card_z_offset;
		for (uint i = 1; i < current_cards.size(); i++) {
			...
			auto &current_render = registry.renderForeground.get(current_card);
			stationary.position = vec2(prev_stationary.position[0] + card_offset,
			                           CARD_AXIS_HEIGHT);
			stationary.scale = vec2(CARD_WIDTH, CARD_HEIGHT);
			current_render.z_position = Z_FOREGROUND - card_z_offset * (i + 2);
		}
	}
}
